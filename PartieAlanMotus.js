function traitement_couleurs(motO,motJ) { //fonction permettant de traiter les couleurs selon les lettres saisies par le joueur.
  ...
  for (i=0; i<=7; i++) {
    ...
    for (j=0; j<=7; j++) {
      if (grille_test[i]==true && T2[i]==T1[j] && j!=i) {  //si la lettre est dans le mot mais au mauvais endroit alors la grille_position sera égale à 2. De plus la grille_test[i] passera à false empechant ainsi la boucle de se répéter.
        grille_position[i]=2;
        grille_test[i]=false;
      }
    }
...
function affichage(mot,ligne){
  ...
  // quand un essai est fait, passer à la ligne suivante de la grille et stocker les caractères
    ...
    if( grille_position[colonne] == 3 ) {    //les lettres correspondant à la grille position 3 ne serons pas colorées (lettres noirs).
        fill(0,0,0);
      }
